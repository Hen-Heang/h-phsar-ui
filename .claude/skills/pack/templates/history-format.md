...
- 예: `## 2026-05-05T17:42+09:00 — app-service @ develop`
...
```markdown
## 2026-05-05T17:42+09:00 — app-service @ develop

### Done
- 입력 유효성 검사 17건 중 CRITICAL 8건 적용
- InputValidationFilter 인터셉터 등록

### In-progress (snapshot)
**Plan**: 검사 적용 범위를 WARNING 등급 9건까지 확장
**Next**:
1. WARNING 9건 분류
2. 검사 적용 PR 분리
**Caution**: 주문 처리 페이지는 별도 검증 필요
...
| 최신 1개 entry (세션 시작 규칙 — 필요 시에만) | `awk '/^## /{c++} c>1{exit} {print}' HANDOFF_HISTORY.md` |
...
예시:
```
- 메타 작업 (Phase B 일괄 - N 프로젝트 CLAUDE.md 압축) 일환 — app-api/CLAUDE.md 312→245줄, -67줄. 알림 정책 표 별도 doc 분리.
```
...
| --------- | -------- | -------------- |
| `app-api/order-api/.../Foo.java` | `app-api` | ✅ (Done 있으면) |
| `app-backoffice/.../bar.xml` | `app-backoffice` | ✅ (Done 있으면) |
