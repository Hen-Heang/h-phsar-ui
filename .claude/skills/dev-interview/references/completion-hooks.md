...
> dev-interview 가 `{{config.outputDir}}/{taskId}_dev_brief.md` Write 직후 사용자에게 노출하는 안내.
...
### `planSkill = dev-plan` (현재 기본값)

```
다음 단계 (즉시 실행 가능):
- /dev-prepare {taskId} — 사람이 먼저 할 선행작업(DDL·설정·권한) + 개발 최소화 방안 제안 (권장)
- /dev-plan {taskId} — 이 브리프 기반 개발 계획서 작성
- /develop {project} — 개발 스코프 설정
- 직접 착수 — 브리프 §11 구현 순서 참고
```

> `/dev-prepare` 를 먼저 돌리면 계획서가 **선행작업 완료를 전제로** 세워진다 — 사람이 해야 할 일이 개발 Task 로 섞여 들어가지 않는다. `planSkill` 을 `dev-prepare` 로 둔 프로젝트는 아래 분기에 따라 그쪽이 먼저 안내된다.
