...
import DistributorProductCard from "../../components/Supplier/DistributorProductCard";
...
const ProductDistributor = () => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.status === 200 || res.status === 204) {
        dispatch(deleteProductDistributor(productToDelete.id));
  ...
  const togglePublish = async (product) => {
    try {
      ...
      if (res.status === 200) {
        dispatch(setUnpublished(product));
        toast.info(product.isPublish ? "Product unpublished" : "Product published");
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-black text-slate-900 mb-2 tracking-tight">Product Catalog</h1>
          <p className="text-slate-500 font-medium">Manage and monitor your distributor inventory.</p>
        </div>
        <Button 
      ...
      <div className="bg-white p-4 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col lg:flex-row gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" className="h-14 px-6 rounded-2xl border-slate-100 bg-slate-50 gap-2 text-slate-600 font-bold hover:bg-slate-100">
      ...
      <AnimatePresence mode="wait">
        {loading ? (
            ...
            >
              {filteredProducts.map((product, index) => (
                <DistributorProductCard 
                  ...
                  product={product}
                  index={index}
                  onEdit={(p) => router.push(`/supplier/update-product?id=${p.id}`)}
                ...
                <tbody className="divide-y divide-slate-50">
                  {filteredProducts.map((product) => (
                    <tr key={product.id} className="group hover:bg-slate-50/50 transition-colors">
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">SKU: {String(product.id).slice(0, 8)}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-5 font-black text-slate-900">${product.price?.toFixed(2)}</td>
                      <td className="px-6 py-5">
                        <div className="flex flex-col gap-1">
                          <span className={`text-sm font-bold ${product.qty < 10 ? 'text-rose-500' : 'text-slate-700'}`}>
                            {product.qty} units
                          </span>
                          <div className="w-24 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                            <div 
                              className={`h-full rounded-full ${product.qty < 10 ? 'bg-rose-500' : 'bg-blue-500'}`} 
                              style={{ width: `${Math.min(product.qty, 100)}%` }}
                            />
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-5">
                        <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider ${
                          ...
                          <span className={`w-1.5 h-1.5 rounded-full ${product.isPublish ? 'bg-emerald-500 animate-pulse' : 'bg-slate-400'}`} />
                      ...
                      <td className="px-8 py-5 text-right">
                        <div className="flex justify-end gap-2">
                          <button 
                            onClick={() => router.push(`/supplier/update-product?id=${product.id}`)}
          ...
          >
            ...
            <p className="text-slate-500 mb-8">Start by adding your first product to the catalog.</p>
            <Button 
              onClick={() => router.push("/supplier/add-product")}
      ...
      <Dialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
        <DialogContent className="max-w-md p-8 rounded-[2.5rem] border-none shadow-2xl">
          <div className="text-center">
            ...
            <DialogTitle className="text-2xl font-black text-slate-900 mb-2">Delete Product?</DialogTitle>
            <p className="text-slate-500 mb-8">This action cannot be undone. This product will be permanently removed from your catalog.</p>
