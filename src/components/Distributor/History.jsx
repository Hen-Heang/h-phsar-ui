...
export default function History() {
  ...
  const stats = useMemo(() => {
    const total = filteredHistory.reduce((sum, item) => sum + (item.total || 0), 0);
    const units = filteredHistory.reduce((sum, item) => sum + (item.qty || 0), 0);
    return { total, units };
  }, [filteredHistory]);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <motion.div variants={itemVariants}>
          <div className="mb-2 flex items-center gap-2 text-blue-600">
            ...
            <span className="text-xs font-black uppercase tracking-[0.2em]">Inventory Log</span>
          </div>
          <h1 className="text-3xl font-black tracking-tight text-slate-900 sm:text-4xl">
            Import History
          </h1>
          <p className="mt-2 text-slate-500 max-w-xl font-medium">
            A comprehensive audit trail of all stock acquisitions and warehouse intake operations.
          </p>
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="flex flex-wrap items-center gap-3">
          ...
          <Button variant="outline" className="h-12 px-6 rounded-2xl bg-white border-slate-200 font-bold gap-2 shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { label: "Total Investment", value: `$${stats.total.toLocaleString()}`, icon: DollarSign, color: "text-emerald-600", bg: "bg-emerald-50" },
          { label: "Units Intake", value: stats.units.toLocaleString(), icon: Package, color: "text-blue-600", bg: "bg-blue-50" },
          { label: "Unique Batches", value: filteredHistory.length, icon: HistoryIcon, color: "text-purple-600", bg: "bg-purple-50" },
        ].map((stat, i) => (
                  ...
                  {loading ? (
                    <tr>
                      <td colSpan={6} className="py-32 text-center">
                        ...
                        <p className="mt-8 text-xs font-black text-slate-400 uppercase tracking-[0.3em] animate-pulse">Syncing Audit Logs</p>
                      </td>
                    </tr>
                    ...
                    <tr>
                      <td colSpan={6} className="py-32 text-center px-8">
                        <div className="flex flex-col items-center max-w-xs mx-auto">
                          ...
                          <p className="text-sm text-slate-500 mt-2">Try adjusting your filters or check back after your next inventory intake.</p>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    <AnimatePresence mode="wait">
                      {currentHistory.map((item, index) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-5">
                            <p className="text-sm font-black text-slate-900 line-clamp-1">{item.name || "Unnamed Product"}</p>
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">SKU: SF-{Math.random().toString(36).substr(2, 6).toUpperCase()}</p>
                          </td>
            ...
            {!loading && pageCount > 1 && (
              <div className="p-8 border-t border-slate-50 flex justify-between items-center">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                  Showing {itemOffset + 1} to {Math.min(endOffset, filteredHistory.length)} of {filteredHistory.length} results
                </p>
                <ReactPaginate
                  pageCount={pageCount}
