  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const dateRes = await get_all_product_sort_by_created_date(id, dispatch);
            ...
            <div className="flex flex-1 flex-col gap-4">
              ...
              <div className="flex flex-wrap gap-6 mt-2">
                ...
                <div className="flex items-center gap-2 text-slate-500">
                  ...
                  <span className="text-sm font-medium">{oneShopData.address}</span>
                </div>

                <div className="flex items-center gap-2 text-slate-500">
                  <Phone className="h-4 w-4 text-orange-500" />
                  <span className="text-sm font-medium">
                    {oneShopData.primaryPhone} {oneShopData.additionalPhone ? `/ ${oneShopData.additionalPhone}` : ''}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              <Button 
                ...
                className={`h-14 rounded-2xl px-6 font-bold transition-all ${
                  oneShopData.isBookmarked ? "bg-red-500 hover:bg-red-600 text-white" : "border-slate-200 hover:border-red-200 hover:bg-red-50 hover:text-red-500"
                }`}
              >
          ...
          {toggleState === 0 && (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56 overflow-hidden rounded-2xl p-1.5 shadow-2xl">
                <DropdownMenuItem onClick={() => handleDropdownChange("all")} className="gap-3 rounded-xl py-3">
              ...
              {toggleState === 0 ? (
                <>
                  ...
                  {selectedOption === "price" && (
                    loadingPrice ? <SkeletonCard /> : <AllProductSortByPrice />
                  )}
                  {selectedOption === "recent" && (
                    loadingAdded ? <SkeletonCard /> : <AllProductSortByDate />
                  )}
                </>
              ) : (
                <Beverage />
