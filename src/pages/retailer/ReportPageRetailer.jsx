...
import { getRetailerReport, setLoading, setError } from "../../redux/slices/retailer/retailerReportSlice";
...
export default function ReportPageRetailer() {
  ...
  const { retailerReport: reportRetailer, loading } = useSelector((state) => state.retailerReport);
  ...
  useEffect(() => {
    document.title = "StockFlow | Analytics Report";
    fetchData();
  }, []);
  ...
  const fetchData = async () => {
    ...
    try {
      const formattedStart = dateRange.startDate ? dayjs(dateRange.startDate).format("YYYY-MM") : "";
      const formattedEnd = dateRange.endDate ? dayjs(dateRange.endDate).format("YYYY-MM") : "";
      
      const res = await get_retailer_report(formattedStart, formattedEnd);
      if (res?.data?.status === 200) {
        dispatch(getRetailerReport(res.data.data));
      } else {
        const errorMsg = res?.response?.data?.detail || "Failed to fetch analytical data";
      ...
      <div className="mx-auto w-[90%] max-w-7xl pt-12">
        ...
        <header className="mb-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 text-slate-500 max-w-xl">
              Gain insights into your spending patterns and order performance to optimize your retail operations.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 bg-white p-3 rounded-[2rem] border border-slate-200 shadow-sm z-10 relative">
            ...
            >
              {isRefreshing ? <RefreshCcw className="h-4 w-4 animate-spin" /> : <Filter className="h-4 w-4" />}
              Apply Filters
            </Button>
          </div>
        </header>
        ...
        {loading && !isRefreshing ? (
          <div className="flex flex-col items-center justify-center py-40 gap-4">
            ...
            <p className="text-sm font-bold text-slate-400 uppercase tracking-widest mt-4">Generating Insights...</p>
          </div>
        ) : (
          <motion.div 
                  ...
                  <Card className="border-none shadow-sm hover:shadow-md transition-shadow rounded-[2rem]">
                    <CardContent className="p-6">
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-wider text-slate-400">Stats</span>
                      </div>
                  ...
                  <Card className="border-none shadow-sm rounded-[2.5rem] overflow-hidden">
                    <CardHeader className="border-b border-slate-50 p-6">
                      <div className="flex items-center gap-3">
                        ...
                        <CardTitle className="text-lg font-black text-slate-900">Order Velocity</CardTitle>
                      </div>
                    </CardHeader>
                    <CardContent className="p-8">
                      <div className="text-center mb-6">
                        <span className="text-4xl font-black text-slate-900">{reportRetailer.totalOrder || 0}</span>
                        <p className="text-xs font-bold text-slate-400 uppercase mt-1">Total Orders</p>
                      </div>
                  ...
                  <Card className="border-none shadow-sm rounded-[2.5rem] overflow-hidden">
                    <CardHeader className="border-b border-slate-50 p-6">
                      <div className="flex items-center gap-3">
                        ...
                        <CardTitle className="text-lg font-black text-slate-900">Category Mix</CardTitle>
                      </div>
                    </CardHeader>
                    <CardContent className="p-8">
                      <div className="text-center mb-6">
                        <span className="text-4xl font-black text-slate-900">{reportRetailer.totalQuantityOrder || 0}</span>
                        <p className="text-xs font-bold text-slate-400 uppercase mt-1">Items Procured</p>
                      </div>
                      <div className="h-[220px]">
                    ...
                    <CardHeader className="border-b border-slate-50 p-8">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <CardTitle className="text-xl font-black text-slate-900">Expense Trend</CardTitle>
                            <p className="text-xs font-medium text-slate-400 mt-1">Monthly procurement expenditure</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Yearly Total</span>
                          <span className="text-2xl font-black text-emerald-600">${reportRetailer.totalYearlyExpense || "0.00"}</span>
                        </div>
                      </div>
                    </CardHeader>
