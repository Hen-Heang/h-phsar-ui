...
import AllShopSkeleton from "@/shared/components/skeletons/AllShop";
...
const AllShopCarousel = () => {
  ...
  const pageCount = Math.ceil((dataShop?.length || 0) / itemsPerPage);
  ...
  const handlePageChange = useCallback(
    (event) => {
      const newOffset =
        (event.selected * itemsPerPage) % (dataShop?.length || 1);
    ...
    (e, item) => {
      ...
      if (item.isBookmarked) {
        remove_bookmark_store(item.id).then(() =>
          dispatch(setUpdateBookmarkStore(item)),
        );
      } else {
          ...
          {currentDataShop.map((item, idx) => (
            ...
            >
              ...
              <div className="relative aspect-[16/10] overflow-hidden rounded-2xl bg-slate-50 ">
                ...
                >
                  <Heart
                    className={`h-4.5 w-4.5 ${item.isBookmarked ? "fill-current" : ""}`}
                  />
                </button>
              </div>
              ...
              <div className="mt-5 px-2 pb-2">
                ...
                <div className="mt-4 flex flex-wrap gap-1.5">
                  {item.categories?.slice(0, 2).map((cat) => (
                    <span
                      key={cat.id}
                      className="inline-flex items-center gap-1 rounded-lg bg-slate-50 px-2 py-0.5 text-[9px] font-bold text-slate-500  "
                    >
