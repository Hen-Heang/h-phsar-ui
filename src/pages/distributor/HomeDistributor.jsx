...
import { Calendar, Filter, Clock, TrendingUp, ShoppingBag, Package, Truck, Hourglass, CheckCircle2 } from "lucide-react";
...
const HomeDistributor = () => {
  ...
  useEffect(() => {
    get_all_activity().then((r) => { if (r?.data?.data) dispatch(getActivityInfo(r.data.data)); });
  }, [dispatch]);
  ...
  const handleDateChange = (newValue) => {
    ...
    if (newValue.startDate) setFormattedStartDate(dayjs(newValue.startDate).format("YYYY-MM"));
    if (newValue.endDate) setFormattedEndDate(dayjs(newValue.endDate).format("YYYY-MM"));
  };
      ...
      <Card className="border-none shadow-sm bg-white ">
        <CardHeader className="pb-4">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-xl font-bold text-slate-900 ">Order Activity</CardTitle>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {[
              ...
              <div key={idx} className="flex flex-col items-center p-5 rounded-2xl bg-slate-50/50  border border-slate-100  transition-all hover:scale-[1.02]">
                ...
                <span className="text-3xl font-black text-slate-900 ">{item.value || 0}</span>
              </div>
            ))}
          </div>
        </CardContent>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            {[
              ...
              <div key={idx} className="p-6 rounded-2xl border border-slate-100  bg-slate-50/30  relative overflow-hidden group">
                <p className="text-slate-500 text-xs font-bold uppercase tracking-wider mb-2">{stat.label}</p>
                <div className="flex items-baseline gap-2">
                  <h3 className="text-3xl font-black text-slate-900  leading-none">{stat.value || 0}</h3>
