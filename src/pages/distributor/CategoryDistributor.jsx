...
import { Search, Plus, Edit2, Trash2, AlertTriangle, Layers } from "lucide-react";
...
export default function CategoryDistributor() {
  ...
  const currentCategories = filteredCategories.slice(itemOffset, itemOffset + 10);
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Organize your products by category</p>
          </div>
        </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          onChange={(e) => { setSearchQuery(e.target.value); setItemOffset(0); }}
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={3} className="py-20 text-center text-slate-400">
                  <div className="flex flex-col items-center justify-center space-y-4">
                    ...
                    <p className="text-slate-500">{searchQuery ? "No categories match your search." : "No categories yet. Add one to get started."}</p>
                    {!searchQuery && (
                       <Button onClick={() => setShowAddCategory(true)} variant="outline" className="mt-2 text-blue-600 border-blue-200 hover:bg-blue-50">
              ...
              currentCategories.map((cat, idx) => (
                <tr key={cat.id} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-6 py-4 text-slate-400 text-xs">{itemOffset + idx + 1}</td>
                  <td className="px-6 py-4 font-medium text-slate-900 capitalize">{cat.name}</td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-2">
                      <button
                        onClick={() => { setGetDataUpdate(cat); setOnChangeDataUpdate(cat.name); setShowUpdateCategory(true); }}
                        className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"
                      >
                        <Edit2 className="w-4 h-4" />
                      </button>
                      <button
                        onClick={() => { setGetDataUpdate(cat); setShowDeleteCategory(true); }}
      ...
      <Dialog open={showDeleteCategory} onOpenChange={setShowDeleteCategory}>
        <DialogContent className="max-w-md text-center p-8">
          ...
          <p className="text-slate-500 text-sm mb-8">
            This will affect products currently using this category. This action cannot be undone.
          </p>
